...
export function RevealView() {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xl animate-fade-in">
            <div className="text-center space-y-6 glass-panel p-8 rounded-3xl border-primary/20 animate-scale-up max-w-2xl w-full mx-auto relative overflow-hidden">
                {/* Background Glow */}
                ...
                <p className="text-white/40 text-xs font-bold uppercase tracking-widest relative z-10">The answer was</p>
                ...
                <div className="flex flex-col items-center gap-6 relative z-10">
                    ...
                    <div>
                        ...
                        <p className="text-2xl md:text-3xl text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary font-bold">
                            {result.artistName}
                        </p>
                    </div>
                </div>
                ...
                <div className="pt-6 border-t border-white/10 relative z-10">
                    <div className="flex justify-center gap-2 mb-4 flex-wrap">
                        {result.playerResults.filter(p => p.wasCorrect).map(p => (
                            <span key={p.playerId} className="text-xs bg-green-500/20 text-green-400 px-2 py-1 rounded font-bold uppercase">
                                {p.pseudo} found it!
                            </span>
                        ))}
                    </div>
